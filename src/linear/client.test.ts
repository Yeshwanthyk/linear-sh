...
class MockLinearClient {
  ...
  commentCreateQueue: Array<{ success: boolean; comment?: { id: string } | null }> = [];
  workflowStatesQueue: Array<{ nodes: Array<{ id: string; name: string; team?: { id: string } | null }> }> = [];
  usersQueue: Array<{ nodes: Array<{ id: string; name: string; email?: string | null }> }> = [];
  ...
  issueUpdate(variables: { id: string; input: Record<string, unknown> }) {
    const next = this.issueUpdateQueue.shift() ?? { success: true, issue: createMockIssue({ id: variables.id, identifier: "ENG-1", title: "updated" }) };
    return Promise.resolve(next);
  }
...
describe("LinearService", () => {
  ...
  test("getIssueDetails resolves labels and names", async () => {
    ...
    expect(details.labels[0]).toEqual({ id: "label-1", name: "Bug", color: "#ff0000" });
  ...
  test("createIssue forwards payload", async () => {
    ...
    client.issueCreateQueue.push({ success: true, issue: createMockIssue({ title: "New issue" }) });
  ...
  test("updateIssue returns updated summary", async () => {
    ...
    client.issueUpdateQueue.push({ success: true, issue: createMockIssue({ title: "Updated" }) });
