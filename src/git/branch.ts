...
export function createBranch(branch: string, options: GitOptions = {}): void {
	const result = spawnSync("git", ["checkout", "-b", branch], {
		cwd: options.cwd,
		stdio: "inherit",
	});
	if (result.status !== 0) {
		throw new GitIntegrationError(`Failed to create branch ${branch}`);
	}
}

export function checkoutBranch(branch: string, options: GitOptions = {}): void {
