...
export const CliContext = Context.GenericTag<CommandContext>("linear-sh/runtime/CliContext");

export function runCommandEffect<R extends typeof CliContext | CommandContext>(
	context: CommandContext,
	program: Effect.Effect<number, unknown, R>,
): Promise<number> {
	const provided = Effect.provideService(program, CliContext, context) as Effect.Effect<
		number,
		unknown,
		never
	>;
	return Effect.runPromiseExit(provided).then((exit) =>
		Exit.match(exit, {
			...
			onFailure: (cause) => {
				...
				const meta = failure instanceof LinearCliError ? { code: failure.code } : undefined;
