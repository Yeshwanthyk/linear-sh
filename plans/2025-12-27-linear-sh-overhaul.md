...
- [x] No `@biomejs/biome` in node_modules after `bun install`
...
- [x] Can create `~/.config/linear-sh/config.json` with profile structure
- [x] `loadConfigSync()` resolves profile from env/file
...
- [x] ConfigService resolves profile from config file
- [x] Cache files written to org-namespaced directory
...
- [x] Commands can extend BaseCommand and use `this.run(program)`
- [x] Layers compose correctly
